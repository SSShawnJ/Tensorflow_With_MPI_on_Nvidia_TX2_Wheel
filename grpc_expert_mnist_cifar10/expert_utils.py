...
def local_moe(x,
  ...
  with tf.variable_scope(name, default_name="local_moe"):
    ...
    expert_devices=['job:worker/task:0','job:worker/task:1','job:worker/task:2','job:worker/task:3']
    ...
    #print("y flat:",y_flat.get_shape())
    #print("y_flat:",y_flat.get_shape())
    # y = common_layers.reshape_like(y_flat, x)
    # print("y shape:",y.get_shape())
    y = tf.reshape(y_flat, [-1, 8, 8, 384])
    if not tf.contrib.eager.in_eager_mode():
      y.set_shape([None, 8, 8, 384])
    #print("y:",y.get_shape())
