...
class Expert ():
    ...
    def run(self):
        ...
        with tf.Graph().as_default():
            ...
            hparams.add_hparam("num_experts", self.params.num_experts)
            Modes = tf.estimator.ModeKeys
...
num_experts must be 1
'''
def fast_predict(params, server, cluster):
...
def get_logger(params):
    logger = logging.getLogger("%s_%s_%s"%(params.mode, params.hparams, 'rs' if params.reshuffle_each_epoch else 'nrs'))
    logger.setLevel(logging.DEBUG if params.verbose else logging.INFO)
    if not logger.hasHandlers():
        # create file handler which logs even debug messages
        fh = logging.FileHandler(join(params.log_dir, "%s_%s_%s.log"%(params.mode, params.hparams, 'rs' if params.reshuffle_each_epoch else 'nrs')))
...
def main(params):   
    ...
    if params.mode == "predict":
        if max_epochs-start_epoch >= 0: 
            fast_predict(params, server, cluster)
