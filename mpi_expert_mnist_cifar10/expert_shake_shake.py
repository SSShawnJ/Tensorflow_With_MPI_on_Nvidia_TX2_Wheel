...
class ExpertShakeShake(t2t_model.T2TModel):
  ...
  def body(self, features):
    ...
    fn = shake_shake_expert_fn(k, n, hparams,is_training)
    x = flatten_image_input(inputs)
    y, loss = expert_utils.local_moe(x, is_training, fn, hparams.num_experts, k = 2)

    print("shake y:",y.get_shape())

    return y, loss


def flatten_image_input(a):
  """Flatten all dimensions of a except the last 3."""
  dim = a.get_shape().as_list()
  #print(dim)
  input_dim = dim[-1] * dim[-2] * dim[-3]

  ret = tf.reshape(a, [-1, input_dim])
  if not tf.contrib.eager.in_eager_mode():
    ret.set_shape([None] + [input_dim])
  return ret

def shake_shake_expert_fn(k, n, hparams, is_training):

  def expert_fn(x):
    #print(x.get_shape())
    x = tf.reshape(x, [-1, 32, 32, 3])
    if not tf.contrib.eager.in_eager_mode():
      x.set_shape([None, 32, 32, 3])
    #print(x.get_shape())
    ...
    #print("fn out:",x.get_shape())
    return flatten_image_input(x)
  return expert_fn

@registry.register_hparams
def expert_shakeshake_small():
...
@registry.register_hparams
def expert_shake_shake_quick():
  hparams = expert_shakeshake_small()
...
@registry.register_hparams
def expert_shakeshake_tpu():
  hparams = expert_shakeshake_big()
...
@registry.register_hparams
def expert_shakeshake_tiny_l26():
  hparams = expert_shakeshake_small()
  hparams.num_hidden_layers = 26
...
@registry.register_hparams
def expert_shakeshake_tiny_l20():
  hparams = expert_shakeshake_small()
...
@registry.register_hparams
def expert_shakeshake_tiny_l14():
  hparams = expert_shakeshake_small()
...
@registry.register_hparams
def expert_shakeshake_small_l26():
  hparams = expert_shakeshake_small()
  hparams.num_hidden_layers = 26
  return hparams

@registry.register_hparams
def expert_shakeshake_small_l20():
  hparams = expert_shakeshake_small()
  hparams.num_hidden_layers = 20
  return hparams

@registry.register_hparams
def expert_shakeshake_small_l14():
  hparams = expert_shakeshake_small()
  hparams.num_hidden_layers = 14
  return hparams
  
@registry.register_hparams
def expert_shakeshake_small_l8():
  hparams = expert_shakeshake_small()
  hparams.num_hidden_layers = 8
  return hparams
