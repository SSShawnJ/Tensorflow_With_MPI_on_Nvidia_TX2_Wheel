...
class ExpertBasicFcRelu(t2t_model.T2TModel):
  ...
  def body(self, features):
    ...
    fn = basic_expert_fn(hparams)

    y, loss = expert_utils.local_moe(x, is_training, fn, hparams.num_experts)
  
    return tf.expand_dims(tf.expand_dims(y, axis=1), axis=1), loss # 4D For T2T.


def basic_expert_fn(hparams):
  def expert_fn(x):
...
@registry.register_hparams
def expert_basic_fc_2():
  """Small fully connected model."""
  hparams = expert_basic_fc_small()
